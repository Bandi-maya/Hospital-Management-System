    ...
    export default function AddPatient({ onAddPatient }: AddPatientProps) {
        ...
        const getExtraFields = async () => {
            ...
            try {
                ...
                if (!data?.error) {
                    setExtraFields(data.data.filter((field: any) => field.user_type_data.type.toUpperCase() === "PATIENT"));
                } else {
        ...
        const getDoctors = async () => {
            ...
            try {
                const data = await getApi(`/users?user_type=DOCTOR`);
                if (!data?.error) {
        ...
        const handleSubmit = async (values: any) => {
            ...
            try {
                ...
                const data = await PostApi(`/users`, newPatient);
                if (!data?.error) {
                    ...
                    <Card
                        ...
                        title={
                            <div className="flex items-center gap-4 p-2">
                                ...
                                <div>
                                    <Title level={3} className="m-0 text-gray-900">Add New Patient</Title>
                                    <Text type="secondary" className="text-base">Fill in all required patient details to create a new record</Text>
                                </div>
                            </div>
                        }
                        extra={
